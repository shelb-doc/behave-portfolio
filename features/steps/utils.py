...
@step("I go to the url path {path}")
def step_impl(context, path):
    """
    :param path: str, Url path to browse to.
    :type context: behave.runner.Context
    """
    context.browser.goto(f'{context.config.userdata["url"]}{path}')
    print(f'Opening {context.config.userdata["url"]}{path}\n')
    sleep(5)
...
def step_impl(context, html_tag, selector, identifier):
    ...
    elif html_tag.lower() == 'id':
        context.browser.id(kwargs).click()
